...
async def save_group(bot, message):
    ...
    if temp.ME in r_j_check:
        if not await db.get_chat(message.chat.id):
            ...
            try:
                ...
                invite_link = "🔒 Could not generate invite link. Bot might not be an admin."
                
            log_msg = script.LOG_TEXT_G.format(message.chat.title, message.chat.id, total, r_j)
            log_msg += f"\n\n🔗 **Group Invite Link:** {invite_link}"
    ...
    [
        ...
        await message.reply_text(    
            text = f"""<b>🎬 ᴛʜᴀɴᴋ ʏᴏᴜ ғᴏʀ ᴀᴅᴅɪɴɢ ᴍᴇ ɪɴ 『{message.chat.title}』 ❣️</b>\n\n
...
async def leave_a_chat(bot, message):
    ...
    try:
        buttons = [[
                  InlineKeyboardButton("📌 ᴄᴏɴᴛᴀᴄᴛ ꜱᴜᴘᴘᴏʀᴛ 📌", url=OWNER_BOTZ)
                  ]]
        reply_markup=InlineKeyboardMarkup(buttons)
...
@Client.on_message(filters.command("reset_primen") & filters.user(ADMINS))
